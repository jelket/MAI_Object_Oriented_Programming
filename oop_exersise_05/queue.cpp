...
class Queue {
private:
    class Node {//Узел очереди
    public:
        Node(T val): next{nullptr}, prev{next}, value{val} {};
        friend class Queue;

    private:
        std::shared_ptr<Node> next;//указатель на след элемент
        std::weak_ptr<Node> prev;
        T value;
    };

public:
    class ForwardIterator {//Класс для итератора
    public:
        using value_type = T;
        using reference = T&;
        using pointer = T*;
    ...
    void Push(const T& val) {//Функция для вставки в начало очереди
        if (!head) {
    ...
    ForwardIterator Insert(const ForwardIterator it, const T& val) { //Вставка по итератору
    ...
    ForwardIterator Erase(const ForwardIterator it) {//Удаление по итератору
        if (it == ForwardIterator{}) {
            return End();
        }
    ...
    ForwardIterator Begin() {//Начало очереди
        return head;
    }

    ForwardIterator End() {//Конец очереди
        return ForwardIterator{};
    }
    ...
    void Print() {//Печать очереди
        ...
        std::for_each(Begin(), End(), [it, this](auto e)mutable{
            std::cout << 0.5 * e.c.first * e.c.second;
            if (it.Next() != this->End()) {
                std::cout << "\n";
            }
            it++;
...
private:
    ...
    std::shared_ptr<Node> head;//Головной элемент
    std::weak_ptr<Node> tail;//Хвостовой элемент
    size_t size;//Размер
};
